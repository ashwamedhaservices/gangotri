...
// Get question paper listing
export const getAllQuestionPapers = async ({ testable_type, testable_id }) => {
  console.log(`[service]::[getAllQuestionPapers]:: `);
  const responseJson = await makeRequest(
      'get', getAllQuestionPapersUrl({ testable_type, testable_id }), {});
  console.log(
      `[service]::[getAllQuestionPapers]::[makeRequests]::[result] ${responseJson}`);
  if (responseJson['success']) {
    return responseJson['data'];
  }
  return {};
...
export const getPayoutReportCsv = async () => {
  console.log('[service]::[getPayoutReportCsv]:: ');
  
  const response = await fetch(getPayoutReportCsvUrl());
  
  if (response.ok) {
    const csvData = await response.blob();
    console.log(`[service]::[getPayoutReportCsv]::[result] ${csvData}`);
    return csvData;
  } else {
    console.error(`[service]::[getPayoutReportCsv]::[error] ${response.status}`);
    return '';
  }
};
